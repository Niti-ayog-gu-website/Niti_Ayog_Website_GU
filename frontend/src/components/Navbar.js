...
const NAV_ADMIN  = ["Home", "Students", "GUStudents", "Alumni", "Research", "Industry"];

// Display labels for nav items that differ from their page key
const NAV_LABELS = {
  GUStudents: "GU Students",
};
...
/>  
          ...
          <div className="nav-links">
            ...
            {links.map(label => {
              ...
              return (
                <button key={label} className={cls} onClick={() => handleNav(label)}>{NAV_LABELS[label] || label}</button>
              );
            })}
