...
const cleanSalary = (raw) => {
  if (!raw) return '';
  // Normalize: remove extra spaces, replace en-dash with hyphen
  let s = raw.replace(/–/g, '-').trim();
  const map = {
    'below ₹2 lpa':      'Below ₹2 LPA',
    'below 2 lpa':       'Below ₹2 LPA',   // handle forms that don't send ₹
    '₹2-5 lpa':          '₹2-5 LPA',
    '2-5 lpa':           '₹2-5 LPA',
    '₹5-8 lpa':          '₹5-8 LPA',
    '5-8 lpa':           '₹5-8 LPA',
      ...
      try {
        ...
        const normalizeCourse = (val) => {
          const map = {
            ...
            'master of science in ai'          : 'MSc AI',
            'master of science in ids'         : 'MSc IDS',
        ...
        const payload = {
          ...
          course:                       normalizeCourse(getIdx(13)),
          year_of_passing: (() => {
            const raw = getIdx(14);
    ...
    for (const row of dataRows) {
      try {
        ...
        const payload = {
          ...
          location:                getIdx(17),
          years_of_experience:     normalizeExperience(getIdx(18)),
